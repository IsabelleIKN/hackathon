...
namespace SG.XR
{
    ...
    public class SG_XR_SimpleControllerSupport : MonoBehaviour, IHandPoseProvider
    {
        ...
        {
            CheckDeviceLink();
            return controllerLinked && LastControllerPos.sqrMagnitude > 0.01f; //not at 0.0.0. Does work because lastControllerPos is relative to the play area
        }
        ...
        public void CalculateWristLocation(out Vector3 wristPos, out Quaternion wristRot)
        {
            ...
            Transform origin = SG_XR_SceneTrackingLinks.GetXRRigTransform();

            Vector3 cPos = LastControllerPos; Quaternion cRot = LastControllerRot;
            SG_HandTracking.CheckOpenXRCompenstation(TrackingHardware.Quest3Controller, this.rightHand, ref cPos, ref cRot);

            // For the others, I need at least the current controller position
            Quaternion controllerRot = origin != null ? origin.rotation * cRot : cRot;
            Vector3 controllerPos = origin != null ? origin.position + (origin.rotation * cPos) : cPos;
